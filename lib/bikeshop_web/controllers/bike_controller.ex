defmodule BikeShopWeb.BikeController do
  ...
  def new(conn, _params) do
    changeset = Inventory.change_bike(%Bike{})
    render(conn, "new.html", changeset: changeset)
  end

  def create(conn, %{"bike" => bike_params}) do
    case Inventory.create_bike(bike_params) do
      {:ok, bike} ->
        conn
  ...
  def update(conn, %{"id" => id, "bike" => bike_params}) do
    ...
    case Inventory.update_bike(bike, bike_params) do
      {:ok, bike} ->
        conn
  ...
  def delete(conn, %{"id" => id}) do
    bike = Inventory.get_bike!(id)
    {:ok, _bike} = Inventory.delete_bike(bike)
